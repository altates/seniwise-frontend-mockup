...
* Do not hard-break lines when writing code.
* Use the shared `labels()` helper for localization (EJS + server); it supports `$1` formatting, `\$` for literal `$`, and returns `INVALID_KEY("...")` when missing.
